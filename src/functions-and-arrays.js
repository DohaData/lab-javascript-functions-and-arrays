...
function sum(genericArray) {
  ...
  for (let i = 0; i < genericArray.length; i++) {
    elementType = typeof genericArray[i];
    if (!allowedTypes.includes(elementType)) {
      throw new Error(`Only ${allowedTypes} are allowed`);
    }
...
function averageWordLength(wordsArray) {
  ...
  wordsLengthArr = wordsArray.map(function (wrd) {
    return wrd.length;
  });
  return sumNumbers(wordsLengthArr) / wordsArray.length;
}
...
function uniquifyArray(wordsUnique) {
  ...
  for (let word of wordsUnique) {
    if (!uniqueWordsResult.includes(word)) {
      uniqueWordsResult.push(word);
    }
  }
